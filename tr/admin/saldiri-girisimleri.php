...
<div class="wrapper">
  ...
  <div style="width: 70%" class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Saldırgan İzleme / Saldırı Girişimleri</h3>
		...
		Bu özelliği aktif ederseniz, saldırı girişiminde bulunan saldırganın IP adresi otomatik olarak engellenir. XSS, SQL injection, code injection gibi URL yoluyla istismar edilebilen birçok saldırıyı otomatik tespit eder ve saldırganı engeller.<br>
		<b><font color="red">ÖRNEK:</font></b> '.strip_tags($_SERVER['SERVER_NAME']).'/icerik?p=PAYLOAD 
		<br>
		<font color="red"><b>DİKKAT:</b></font> Bu özellik, bazı arama motorlarının engellenmesine de sebep olabilir.<br><br>
<center><form action="" method="post">
<div class="form-group">
<div class="custom-control custom-checkbox">
      <input name="saldiritespiti" type="checkbox" class="custom-control-input" id="customCheck1">
      <label class="custom-control-label" for="customCheck1">Saldırı tespitini ';
$uisorgu3 = $dbbaglan->query('SELECT * FROM guvenlik', PDO::FETCH_ASSOC);
if ( $uisorgu3->rowCount() ){
foreach( $uisorgu3 as $guvtbl ){
if($guvtbl['saldirgantespit'] == 0){
		echo '<font color="green"><b>AKTİF</b></font> et</label>';
		if($_POST['saldiritespiti'] and $_POST['csrftoken'] == $_SESSION['csrftoken']){
		$dbbaglan->query("UPDATE guvenlik SET saldirgantespit='1'");
	...
	if($_POST['saldiritespiti'] and $_POST['csrftoken'] == $_SESSION['csrftoken']){
		$dbbaglan->query("UPDATE guvenlik SET saldirgantespit='0'");
		echo '<script>window.location.assign(document.URL);</script>';
	}else{
	$_SESSION['csrftoken'] = md5(rand());	
	echo '<input type="hidden" name="csrftoken" value="'.$_SESSION['csrftoken'].'">';		
...
<input type="submit" style="width: 40%" class="btn bg-purple btn-flat margin" value="Kaydet"></form></div></center>
...
$uisorgu2 = $dbbaglan->query('SELECT * FROM ipban where aciklama = "saldiri-girisimi"', PDO::FETCH_ASSOC);
...
echo '
<td>'.$ipcek.'</td>
<td><input type="hidden" name="ipburada" value="'.$ipcek.'"><input type="submit" style="width: 20%" value="Kaldır" name="bankaldir" class="btn bg-purple btn-flat margin">
