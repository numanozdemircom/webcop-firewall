...
<div class="wrapper">
  ...
  <div style="width: 70%" class="box">
        <div class="box-header with-border">
          <h3 class="box-title">URL Yönlendir</h3>
		...
		<form action="" method="post">Sitenizi başka bir URL\'e yönlendirebilirsiniz. Aşağıdaki alana yönlendirilmesini istediğiniz adresi yazın.<br><br>
<center><input type="text" name="urlgir" class="form-control" style="width: 40%" value="';
}
}
$uisorgu2 = $dbbaglan->query('SELECT * FROM ekstra', PDO::FETCH_ASSOC);
if ( $uisorgu2->rowCount() ){
foreach( $uisorgu2 as $extbl ){
	echo htmlspecialchars($extbl['yeniurl']);	
}
}
	echo '"><br>
<div class="form-group">
<div class="custom-control custom-checkbox">
      <input name="urlyon" type="checkbox" class="custom-control-input" id="customCheck1">
      <label class="custom-control-label" for="customCheck1">URL yönlendirme özelliğini ';
$uisorgu3 = $dbbaglan->query('SELECT * FROM guvenlik', PDO::FETCH_ASSOC);
if ( $uisorgu3->rowCount() ){
foreach( $uisorgu3 as $guvtbl ){
if($guvtbl['urlyonlendir'] == 0){
	// sıkıntı var, post edildigi an devredisi veya aktif diyor, header ile yonlendir
		echo '<font color="green"><b>AKTİF</b></font> et</label>';
		if($_POST['urlyon'] and $_POST['csrftoken'] == $_SESSION['csrftoken']){
if(filter_var($_POST['urlgir'], FILTER_VALIDATE_URL)){
		$engellenecekler = array("'", '"', "\\", ";", ">", "<", "data:", "javascript:", "@");
		$urlguncelle = str_replace($engellenecekler, "", strtolower($_POST['urlgir']));
		$dbbaglan->query("UPDATE ekstra SET yeniurl='$urlguncelle'");
		$dbbaglan->query("UPDATE guvenlik SET urlyonlendir='1'");
		echo '<script>window.location.assign(document.URL);</script>';
		}else{
		echo "<br><font color='red'><b>Geçerli bir URL girmediniz.</b></font>";
		}
	}else{
...
if(filter_var($_POST['urlgir'], FILTER_VALIDATE_URL)){
		...
		$urlguncelle = str_replace($engellenecekler, "", strtolower($_POST['urlgir']));
		$dbbaglan->query("UPDATE ekstra SET yeniurl='$urlguncelle'");
		$dbbaglan->query("UPDATE guvenlik SET urlyonlendir='0'");
		echo '<script>window.location.assign(document.URL);</script>';
		}else{
		echo "<br><font color='red'><b>Geçerli bir URL girmediniz.</b></font>";
		}
