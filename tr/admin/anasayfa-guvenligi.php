...
<div class="wrapper">
  ...
  <div style="width: 70%" class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Anasayfa Güvenliği</h3>
		...
		<form action="" method="post">Sitenizin anasayfasında (index.html ve index.php dosyaları) <b>"hacked, h4cked, h4ck3d, owned, own3d, 0wn3d, hack3d, pwned, pwn3d, rooted, r00ted, hackle"</b> kelimeleri tespit edildiğinde sistem bunu saldırı olarak algılar ve aşağıdakileri yerine getirir:<br>
		<li>Saldırıya uğrayan sayfanın otomatik yedeğini alır, daha sonra siteyi bakım moduna alarak anasayfaya "bakımdayız" mesajı basar, böylece sitenizi ziyaret edenler saldırı sayfasını göremez. 
		<li>Yedeklerin alındığı klasörde ('.strip_tags($ayartbl["dizinwcp"]).'/yedek), ilgili saldırı sayfalarının yedeğini bulabilirsiniz. Bu yedekler zaman damgalıdır ve yedeği alınan dosyalar rastgele isimlerle adlandırılmaktadır. 
		<li>Ayrıca, saldırı yapıldığında sunucuda mail(); fonksiyonu açıksa sistemde kayıtlı olan e-mail adresinize saldırı geldiğinine dair bildirim gönderilir.
		<br>
<center><br>
...
if($guvtbl['anasayfaguvenligi'] == 0){
	...
	echo '<font color="red"><b>DEAKTİF</b></font> et</label><br><br>
	Bu özelliği aktif ettiyseniz sunucu kontrol paneli veya SSH aracılığıyla bir cron job oluşturmalısınız. Bir kere oluşturulması yeterlidir, bu özelliği devredışı bırakmak istediğinizde WebCOP panelden bu özelliği kapatabilirsiniz, cron\'u silmeniz gerekmez. Cron job mutlaka <u>dakikada bir</u> çalışacak şekilde ayarlanmalıdır ve aşağıdaki komut girilmelidir:<br>
	<pre>php -q '.strip_tags($ayartbl["dizinwcp"]).'/lib/index-guvenligi.php</pre>
	';
	if($_POST['indexguv'] and $_POST['csrftoken'] == $_SESSION['csrftoken']){
